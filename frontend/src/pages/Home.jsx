...
export default function Home() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("Alle Verträge");

  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const url = selectedCategory === "Alle Verträge" 
          ? '/products' 
  ...
  const trendingContracts = products.filter(p => p.type === 'Cloud Vertrag').slice(0, 5);
  const hardwareDeals = products.filter(p => p.type === 'Hardware' || p.type === 'Hardware + Hosting').slice(0, 5);

  return (
    <div className="bg-[#0f0f10] min-h-screen font-sans text-zinc-300">
      <Header selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} />
      <main>
        {selectedCategory === "Alle Verträge" && (
          <>
            <Hero />
        ...
        {loading ? (
          ...
          <>
            {selectedCategory === "Alle Verträge" ? (
              <>
                <ProductGrid title="Beliebte Verträge" products={trendingContracts} />
                <ProductGrid title="Hardware & Hosting" products={hardwareDeals} />
                ...
                <ProductGrid title="Alle Produkte ansehen" products={products} />
              </>
            ) : (
              <ProductGrid title={`Kategorie: ${selectedCategory}`} products={products} />
            )}
          </>
        )}
